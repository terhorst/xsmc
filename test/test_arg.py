...
def test_matching_arg_ts_lengths():
    ts = msp.simulate(
        sample_size=2, length=1e5, mutation_rate=1e-3, recombination_rate=1e-3
    )
    focal = 0
    panel = [1]
...
def test_arg_finite_coalescent(rng, sample_size):
    ...
    w = 1e3
    ts = msp.simulate(
        sample_size=sample_size, length=L, mutation_rate=1e-3, recombination_rate=1e-3
    )
    focal = 0
    ...
    for p in panel:
        ...
        n = tc.nodes.add_row(flags=tskit.NODE_IS_SAMPLE, individual=i, time=0.0)
...
def test_scaffolding(rng, sample_size):
    ...
    ts = msp.simulate(
        sample_size=sample_size, length=10, mutation_rate=1e-1, recombination_rate=1e-1
    )
...
def test_thread_func(rng, sample_size):
    ...
    for focal in range(1, sample_size):
        ...
        seg = viterbi_path(
            ts, focal, panel, scaffold, KINGMAN, 1e-1, 1e-1, None, False, 1
        )
        ...
        for t in scaffold.trees():
            ...
            while (
                t.interval[0] <= new_t.interval[0] <= new_t.interval[1] <= t.interval[1]
            ):
